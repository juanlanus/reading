  ...
  var
    ...
    $.extend( Plugin.prototype, {
      ...
      init: function () {
        ...
        function initData() {
          var data = {
            // DEBUG: this should come from the server
        ...
        RT.buildTopsMap();
          ...
          // recursively visit the elements of the DOM tree level and record their docPath data
            ...
            for( i = 0; i < n; i++ ) {
              ...
              if( checkHeading.test( $this[0].tagName ) ) { // it's a header h1...6
                // update header count
                var headerLevel = parseInt( $this[0].tagName.substring(1, 2), 10 );       // this line triggers the loop
                if( headerLevel > 0 && headerLevel < 7 ) {
                  ...
                  // truncate the path array to only the headers count
                  docPath.length = 7;
              ...
              if ( RT.settings.debug || ( docPath && docPath.indexOf( ',,' ) !== -1 ) ) {
                ...
                if( !( tn === 'BR' || tn === 'CODE' || tn === 'COL' || tn === 'A' || tn === 'TD' || tn === 'COLGROUP' || tn === 'TH' || tn === 'TBODY') ) {
                  ...
                  if( !!$this.attr('id') ) { z += '  id:' + $this.attr('id'); }
      ...
      // the RT.data.headerId associative array
        var RT = $.fn.rt.RT;
        RT.data.docMap = {};
        for( var oneHeader in RT.data.headerId ) { 
          ...
          // DEBUG: console.log( oneHeader + ' ' + RT.data.headerId[oneHeader].id );
        };
      },
      ...
      scrollToPosition: function( scrollData ) {
        ...
        if( scrollData && scrollData.dp ) {
          ...
          RT.data.headerId = null;
          if( !! scrollData.id ) {
            // the target itself has an id
            RT.data.headerId = scrollData.id;
          } else {
            // use docPath to get the id of the closest header using
            // the RT.data.headerId[ <here> ] map
            // separate docpath first 7 items (headers)
            var dpHeaders = scrollData.dp.split(',').slice(0, 7).toString();
            var theHeader = RT.data.headerId[ dpHeaders ];
            if( theHeader ) {
              RT.data.headerId = RT.data.headerId[ scrollData.dp.split(',').slice(0, 7).toString() ].id;
            } else {
              return;
            };
          };
          var scrollTarget = document.getElementById( RT.data.headerId );
          if( scrollTarget ) {
            ...
            if( scrollData.dp ) {
              ...
              RT.scrollToElement( $targetElement, this.settings.scrollDuration );
            };
          };
        };
      ...
      smartScrollGetNextTopElement: function( nodePrevIdx ) {
        ...
        while( true ) { 
          ...
          if( (RT.data.topsMap[nodeNextIdx].top - RT.topsMap[nodePrevIdx].top) > RT.smartScrollHeight ) {
            break;
          };
        };
        ...
        RT.scrollToElement( $( nextNode ), RT.settings.scrollDuration * propDelay );
        return nodeNextIdx; 
      },
      ...
      getCurrentReadingPosition: function( event ) {
        ...
        if (( $(topElement).offset().top < 0 ) && ( $(topElement).height() > RT.data.tallElementLimit )) {
          RT.data.scrollData.p = Math.round( Math.abs( $(topElement).offset().top 
    ...
    /* define public functions thus: 
       PESOS.fn.rt.format = function( txt ) {
