...
var RT = {
  ...
  // TOC: the document's table of content and its related methods
  // TOC: {},
  ...
  scrollPosition: 0,                  // the user reading position, a number of px below
                                      // viewport top
  tagCount: 0,                        // number of tags (informative)
  lastElement: null,                  // ref to the document's last element
  ...
  actionsCount: 0,                    // number of action records stored in localStorage
  actionsCountLimit: 100,             // number of actions before sending to server
  actionsTimeLimit: 60000,            // time before sending the actions to the server (ms)
...
// adds a format() method to the Date object
Date.prototype.format = function(format) //author: meizz
{
  ...
  if(/(y+)/.test(format)) format = format.replace(
    RegExp.$1,
...
RT.getRelativeTop = function( elem ) {
...
RT.getCurrentReadingPosition = function(e) {
...
// recursively visit the elements of a DOM tree level and record their docPath data
  ...
  $context.children().not('[float=left]').not('[float=right]').each(
    ...
    // build the docPaths of the subtree of this
      ...
      RT.buildPath ($this, docPath.slice(0), level + 1);
      // count of tags in this document
      RT.tagCount++;
      // this might be the document's last element
      RT.lastElement = this;
...
RT.storeHeaderScrollTops = function() {
...
RT.scrollToElement = function( $topElement, duration ) {
...
RT.drillDown = function( startElement, steps, animDuration ) {
...
RT.isBlockElement = function( theElement ) {
...
RT.smartScrollGetPrevTopElement = function( nodePrevIdx ) {
...
RT.smartScrollGetNextTopElement = function( nodePrevIdx ) {
...
RT.smartScrollToPrevElement = function() {
...
RT.smartScrollToNextElement = function() {
...
/* RT - set it to work ********************************************************/
  ...
  function rt(rtContainerSelector, pageNumber){
    ...
    // TODO: adjust the index below, should be zero
    RT.$content = $(RT.$container.children()[0]);
    if( RT.$container.length !== 1 ) {
      // TODO: should silently choose the taller one
      console.log('The container selector "' + rtContainer + '" matches '
      + RT.$container.length + ' elements, should match one and only one');
      return false;
    }
